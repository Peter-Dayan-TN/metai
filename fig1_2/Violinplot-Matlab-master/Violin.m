classdef Violin < handle
    ...
    methods
        function obj = Violin(data, pos, varargin)
            ...
            [density,value]=ksdensity(data,'support',[.4999 1.0001]);
            value=[min(value)-5e-6 value max(value)+5e-6];
            density=[0 density 0];
            ...
            if ~isempty(data2)
                ...
                [densityC,valueC]=ksdensity(data2,'support',[.4999 1.0001]);
                valueC=[min(valueC)-5e-6 valueC max(valueC)+5e-6];
                densityC=[0 densityC 0];
                area=sum(diff(value).*(density(1:(end-1))+density(2:end))/2);
                area2=sum(diff(valueC).*(densityC(1:(end-1))+densityC(2:end))/2);
                width=1*length(data)/(length(data)+length(data2));
                widthC=1*length(data2)/(length(data)+length(data2));
                norm=(area*width + area2*widthC);
                width=0.03*width/norm;
                widthC=0.03*widthC/norm;
            end
